...
async def local_news(
    ...
    pincode: Optional[str] = Query(None, min_length=3, max_length=10),
    ...
    return await fetch_news(q=q, country="in", language=language, region="Uttar Pradesh", size=size)


@app.get("/api/local-news/{pincode}")
async def local_news_by_pin(pincode: str, size: int = Query(10, ge=1, le=50)):
    return await local_news(location="Mandi Dhanaura", pincode=pincode, language="en", size=size)
